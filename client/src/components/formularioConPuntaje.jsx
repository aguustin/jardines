...
export default function FormularionConPuntaje({ userCUIL }) {
  const [preguntasA, setPreguntasA] = useState(preguntasIniciales);
  const [preguntasB, setPreguntasB] = useState(preguntasSiNo);
  const [sent, setSent] = useState(false);

  const onSelectA = (questionId, optionId) => {
    setPreguntasA(prev =>
      prev.map(q =>
        q.id === questionId
  ...
  const onToggleB = (questionId, checked) => {
    setPreguntasB(prev =>
      prev.map(q =>
        q.id === questionId
          ? { ...q, selected: checked }
  ...
  const handleSubmit = () => {
    const scoreA = preguntasA.reduce((total, q) => {
    ...
    const scoreB = preguntasB.reduce((total, q) => {
      if (q.selected !== null) {
    ...
    alert(`Formulario enviado\nPuntaje Sección A: ${scoreA}\nPuntaje Sección B: ${scoreB}\nPuntaje Total: ${totalScore}`);
  ...
  const answeredA = preguntasA.filter(q => q.selectedOptionId !== null).length;
  const answeredB = preguntasB.filter(q => q.selected !== null).length;
  const totalAnswered = answeredA + answeredB;
  const totalQuestions = preguntasA.length + preguntasB.length;
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-purple-50 py-8 px-4">
      <div className="max-w-7xl mx-auto">
        ...
        {/* Sección A - Preguntas con opciones múltiples */}
        <Card className="mb-6">
          <CardHeader className="bg-gradient-to-r from-blue-500 to-blue-600">
            <CardTitle className="text-white text-xl">Sección A - Información General</CardTitle>
          </CardHeader>
          <CardContent className="p-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {preguntasA.map((q, index) => (
                <div 
                  key={q.id} 
                  className="bg-gradient-to-br from-white to-gray-50 rounded-xl shadow-sm p-5 border-2 border-gray-100 hover:border-blue-300 transition-all"
                >
                  <div className="flex gap-4">
                    ...
                    <div className="flex-1">
                      ...
                      <Select
                        ...
                        onValueChange={value => onSelectA(q.id, value)}
                      >
                        {q.options.map(o => (
        ...
        {/* Sección B - Preguntas Sí/No */}
        <Card>
          <CardHeader className="bg-gradient-to-r from-purple-500 to-purple-600">
            <CardTitle className="text-white text-xl">Sección B - Características del Hogar y Familia</CardTitle>
          </CardHeader>
          <CardContent className="p-6">
        ...
        <div className="mt-8 bg-white rounded-2xl shadow-sm p-6 border border-gray-100">
          <button
            ...
            className={`w-full py-4 rounded-xl font-bold text-lg transition-all transform ${
              !sent && totalAnswered === totalQuestions 
